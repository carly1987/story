...
pub fn create(text: &str, story: i32, chapter: i32) -> i32 {
    ...
    match client.execute(
        ...
        Ok(_) => {
            id = get(text).id;
          println!("<<<<<<<<<<<success: createContent {:?}", id);
...
fn get(text: &str) -> Content {
    let client = connect();
    let mut stmt = client.prepare("SELECT * FROM contents WHERE text = '?1'").unwrap();
...
pub fn update(id: String, text: String) {
    let client = connect();
...
pub fn query(chapter: i32, story: i32) -> Vec<Content> {
    let client = connect();
    let mut stmt = client.prepare("SELECT * FROM contents Where story = ?1 And chapter = ?2 ORDER BY id asc").unwrap();
